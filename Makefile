...
ifeq ($(ODH_PLATFORM_TYPE), OpenDataHub)
	...
	KUBEBUILDER_PROJECT_NAME=opendatahub-operator
	...
	KUBEBUILDER_PROJECT_NAME=rhods-operator
...
.PHONY: prepare
prepare: projectfile manifests kustomize manager-kustomization

.PHONY: projectfile
projectfile: PROJECT.in
	cp PROJECT.in PROJECT
	sed -i 's/projectName: .*/projectName: $(KUBEBUILDER_PROJECT_NAME)/' PROJECT
...
bundle: prepare operator-sdk ## Generate bundle manifests and metadata, then validate generated files.
	$(OPERATOR_SDK) generate kustomize manifests --input-dir $(KUSTOMIZE_DIR) --output-dir $(KUSTOMIZE_DIR) -q
	$(KUSTOMIZE) build $(KUSTOMIZE_DIR) | $(OPERATOR_SDK) generate bundle $(BUNDLE_GEN_FLAGS) --kustomize-dir $(KUSTOMIZE_DIR) --output-dir $(BUNDLE_DIR) 2>&1 | grep -v $(WARNINGMSG)
