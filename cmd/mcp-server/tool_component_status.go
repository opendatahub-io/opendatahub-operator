...
func registerComponentStatus(s *server.MCPServer, kubeClient client.Client) {
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if err != nil {
			log.Printf("component_status: managed resources for %q: %v", component, err)
			return mcp.NewToolResultError(fmt.Sprintf("failed to fetch managed resources for component %q", component)), nil
		}
		...
		if err != nil {
			log.Printf("component_status: json marshal: %v", err)
			return mcp.NewToolResultError("failed to format component status"), nil
		}
