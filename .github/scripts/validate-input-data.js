...
module.exports = async ({ core, context }) => {
    try {
        ...
        // Validate each component entry has required fields
        for (const [componentName, componentData] of Object.entries(parsed)) {
            if (typeof componentData !== 'object' || componentData === null) {
                core.setFailed(`Component "${componentName}" must be an object`);
                return;
            }

            if (!componentData.ref || typeof componentData.ref !== 'string') {
                core.setFailed(`Component "${componentName}" missing required "ref" field (must be a non-empty string)`);
                return;
            }

            if (componentData.ref.trim() === '') {
                core.setFailed(`Component "${componentName}" has empty "ref" field`);
                return;
            }
        }
