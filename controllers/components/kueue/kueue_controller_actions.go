...
const (
	MultiKueueCRDMessage = "MultiKueue CRDs MultiKueueConfig v1alpha1 and/or MultiKueueCluster v1Alpha1 exist, please remove them to proceed"
)

func checkPreConditions(ctx context.Context, rr *odhtypes.ReconciliationRequest) error {
	rConfig, err := cluster.HasCRDWithVersion(ctx, rr.Client, gvk.MultiKueueConfigV1Alpha1.GroupKind(), gvk.MultiKueueConfigV1Alpha1.Version)
	if err != nil {
		return odherrors.NewStopError("failed to check %s CRDs version: %w", gvk.MultiKueueConfigV1Alpha1, err)
	}

	rCluster, err := cluster.HasCRDWithVersion(ctx, rr.Client, gvk.MultikueueClusterV1Alpha1.GroupKind(), gvk.MultikueueClusterV1Alpha1.Version)
	if err != nil {
		return odherrors.NewStopError("failed to check %s CRDs version: %w", gvk.MultiKueueConfigV1Alpha1, err)
	}
