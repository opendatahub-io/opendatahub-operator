...
func copyGroups(ctx context.Context, rr *odhtypes.ReconciliationRequest) error {
	...
	adming := foundGroups["adminGroups"]
	if len(adming) != 0 {
		for _, e := range strings.Split(adming, ",") {
			e = strings.TrimSpace(e)
			// If the group is already in the list then ignore it
			if !slices.Contains(ai.Spec.AdminGroups, e) {
				ai.Spec.AdminGroups = append(ai.Spec.AdminGroups, strings.TrimSpace(e))
				newGroups = true
			}
		}
	}

	allowedg := foundGroups["allowedGroups"]
	if len(allowedg) != 0 {
		for _, e := range strings.Split(allowedg, ",") {
			// If the group is already in the list then ignore it
			if !slices.Contains(ai.Spec.AllowedGroups, e) {
				ai.Spec.AllowedGroups = append(ai.Spec.AllowedGroups, strings.TrimSpace(e))
				newGroups = true
			}
		}
	}
