...
| `releases`           | `[]ComponentRelease`   | Installed component versions (`name`, `repoUrl`, `version`). The platform reads the entry with `name: "platform"` to complete the [platform version handshake](#platform-version-handshake) — the module writes the platform version here after completing upgrade work. Additional entries track sub-component versions. |
...
| **Image config**    | Controller image override + operand image declarations                | `ModuleConfig.ControllerImage` + `ModuleConfig.RelatedImages`|
...
The platform injects into the Deployment's target container after rendering
and before deploy. It overrides the container `image:` field (via
`ModuleConfig.ControllerImage`) and adds `RELATED_IMAGE_*` plus
`APPLICATIONS_NAMESPACE` environment variables. The target container name
defaults to `manager` (the kubebuilder convention) and can be overridden per
module via `ModuleConfig.ContainerName`. If the target container is not
found, injection is skipped and an error is logged. See
[Platform Injection Mechanisms](#platform-injection-mechanisms). You do not
need to declare these env vars in the manifest; the platform adds them
automatically based on the handler's `ModuleConfig`.
    ...
    return &handler{
        BaseHandler: modules.BaseHandler{
            Config: modules.ModuleConfig{
                ...
                ControllerImage: "RELATED_IMAGE_ODH_MYMODULE_OPERATOR_IMAGE",
...
| `GetControllerImage()`      | Returns `Config.ControllerImage` (used by `injectModuleEnv` to override the Deployment container image)                                                    |
| `GetRelatedImages()`        | Returns `Config.RelatedImages` (used by `injectModuleEnv` to inject `RELATED_IMAGE_*` env vars)                                                            |
| `GetDeploymentName()`       | Returns `Config.DeploymentName` (identifies which rendered Deployment receives injection; empty = auto-detect)                                              |
| `GetContainerName()`        | Returns `Config.ContainerName` or `"manager"` (identifies which container in the Deployment receives injection)                                            |
| `GetInitContainerName()`    | Returns `Config.InitContainerName` (optional init container to override with the same controller image)                                                    |
...
- `releases` (array of `ComponentRelease` with `name`, `repoUrl`,
`version`): installed component versions. The entry with `name: "platform"`
participates in the [platform version handshake](#platform-version-handshake)
— the module controller writes the platform version here after completing
all upgrade-related work. The platform reads it to confirm the handshake.
Additional entries track sub-component versions managed by the module.
...
The platform automatically creates a per-module ConfigMap named
`odh-<modulename>-config` for every enabled module, where
`<modulename>` is the module's registered name (the value returned by
`GetName()` / `ModuleConfig.Name`). For example, a module registered
as `"mymodule"` gets `odh-mymodule-config`. This ConfigMap contains
platform-managed fields that the module controller reads to participate
in the [platform version handshake](#platform-version-handshake).

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: odh-mymodule-config
...
**Merge behavior:** If your module's Helm chart or Kustomize overlay
already includes a ConfigMap with the same name
(`odh-<modulename>-config`), the platform merges its managed keys
into the existing ConfigMap. Module-owned keys are preserved; only
platform-managed keys (`platformVersion`) are set or overwritten.

**Enforcement:** The platform applies this ConfigMap via Server-Side
Apply on every reconcile. If an external actor modifies
platform-managed keys, they are reconciled back to the correct values
on the next cycle.

**Usage:** Module operators should name their optional controller
configuration ConfigMap `odh-<modulename>-config` if they want the
platform version to be co-located with their controller flags.
Alternatively, they can use a separate ConfigMap name for controller
config and let the platform create `odh-<modulename>-config`
independently — the platform creates it regardless.

The module controller reads `platformVersion` from this ConfigMap to
complete the version handshake. See
[Platform version handshake](#platform-version-handshake) for the full
protocol.
...
## Platform Injection Mechanisms

The platform operator injects three things into each module operator's
Deployment after rendering and before Server-Side Apply. These injection
points are how the platform controls which images a module runs, which
images it passes to its operands, and how it discovers the shared platform
namespace.


| Injection point | `ModuleConfig` field | What it does |
|-----------------|---------------------|--------------|
| Controller image override | `ControllerImage` | Replaces the `image:` field on the module operator container (and optionally a named init container) with the release-pinned digest |
| Operand image env vars | `RelatedImages` | Injects `RELATED_IMAGE_*` environment variables for images the module operator passes to sub-controllers, sidecars, or workloads it creates |
| Namespace injection | _(automatic)_ | Injects `APPLICATIONS_NAMESPACE` env var so the module operator knows where the shared platform namespace is |


### Why these injection points exist
...
1. **Override** the module operator's container image with the
   release-specific digest (`ControllerImage`).
2. **Forward** the subset of operand image variables the module needs
   (`RelatedImages`).
3. **Inject** the platform namespace (`APPLICATIONS_NAMESPACE`).
...
Platform operator pod
  env:
    RELATED_IMAGE_ODH_AI_GATEWAY_OPERATOR_IMAGE=quay.io/...@sha256:aaa
    RELATED_IMAGE_ODH_BATCH_GATEWAY_OPERATOR_IMAGE=quay.io/...@sha256:bbb
    RELATED_IMAGE_ODH_TRAINER_IMAGE=quay.io/...@sha256:ccc
    ... (all module images)
        │
        │  provisionModules
        │  ├── handler.GetControllerImage() → collect controller image name
        │  ├── handler.GetRelatedImages()   → collect operand image names
        │  └── store on rr.ModuleEnvInjection (scoped per Deployment)
        │
        ...
        │  ├── ControllerImage:   os.Getenv() → override container image field
        │  ├── InitContainerName: override init container image (same value)
        │  ├── RelatedImages:     os.Getenv() for each → inject as env vars
        │  └── APPLICATIONS_NAMESPACE → inject as env var
        │
        │  deploy (SSA apply)
        ▼
Module operator pod (e.g. ai-gateway-operator)
  container "manager":
    image: quay.io/...@sha256:aaa              ← from ControllerImage
    env:
      RELATED_IMAGE_ODH_BATCH_GATEWAY_OPERATOR_IMAGE=quay.io/...@sha256:bbb
...
### Controller image override (`ControllerImage`)

The `ControllerImage` field names the `RELATED_IMAGE_*` env var whose value
should **replace** the module operator Deployment's container `image:` field.
This is how the platform pins the module operator binary to a release-specific
digest from the OLM CSV.

**This is distinct from `RelatedImages`.** `ControllerImage` mutates the
container spec `image` field directly. `RelatedImages` adds env vars. The
controller image is NOT injected as an env var -- it overrides the image the
container actually runs.

**When to set it:** Always, unless the module's Helm chart or Kustomize overlay
already bakes in the correct digest-pinned image (rare -- this only applies to
community/development setups where the chart default is acceptable).

**`InitContainerName`:** Optional companion field. If the module operator's
Deployment includes an init container that shares the same operator binary
(e.g., a `copy-manifests` init container that copies bundled manifests into a
shared volume), set `InitContainerName` to that container's name. The platform
will override its `image:` field with the same resolved value as the main
container.
...
In this example:
- `ControllerImage` overrides the `ai-gateway-operator` container image with
  the digest-pinned reference from the CSV.
- `InitContainerName` overrides the `copy-manifests` init container image with
  the same digest.
- `RelatedImages` injects the four operand images as env vars so the
  ai-gateway-operator can pass them to the sub-module (batch-gateway-operator)
  Deployments it creates.

---

### Operand image env vars (`RelatedImages`)

The `RelatedImages` field lists `RELATED_IMAGE_*` env var names for images the
module operator **forwards to its operands** -- sub-controllers, sidecars,
workload pods, or any resource the module operator creates that needs an image
reference. These are injected as environment variables on the module operator's
target container.
...
The module operator reads these from its process environment using
`os.Getenv()` and uses them when constructing operand resources:

```go
func getTrainingImage() string {
    if img := os.Getenv("RELATED_IMAGE_ODH_TRAINING_CUDA128_TORCH29_PY312_IMAGE"); img != "" {
        return img
    }
    return "quay.io/opendatahub/training-cuda:latest"
}
```
...
operator's own defaults to take effect.

---

### Namespace injection (`APPLICATIONS_NAMESPACE`)

The platform always injects `APPLICATIONS_NAMESPACE` into the module
operator's target container. Module operators use this to discover the shared
platform namespace (e.g., `opendatahub` or `redhat-ods-applications`) for
cross-namespace resource discovery or for deploying workloads that interact
with other modules.

No handler configuration is needed -- this injection is automatic for all
modules.

---

### Deployment targeting (`DeploymentName`, `ContainerName`)

The platform must identify which Deployment and which container within it to
inject into. Two `ModuleConfig` fields control this:

**`DeploymentName`** -- the `metadata.name` of the module operator's
Deployment as rendered (after any Kustomize namePrefix/nameSuffix or Helm
release templating). This is the Deployment the platform injects into.

Fallback logic when `DeploymentName` is empty:
- **Helm modules:** defaults to the Helm release name (`ReleaseName`).
- **Kustomize modules:** defaults to the module handler name (`Name`).

Set `DeploymentName` explicitly when the rendered Deployment name differs from
these defaults. Without it, env injection silently targets the wrong
Deployment and no images are injected.

**`ContainerName`** -- the name of the target container within the Deployment.
Defaults to `"manager"` (the kubebuilder convention). Override only if the
module chart uses a different container name for the operator binary.
...
**Putting the controller image in `RelatedImages` instead of
`ControllerImage`:**

```go
// WRONG -- the Deployment still runs the chart's default image tag.
// The image reference is only injected as an env var, which the
// module operator may or may not read.
RelatedImages: []string{
    "RELATED_IMAGE_ODH_MY_MODULE_OPERATOR_IMAGE",
}

// CORRECT -- the platform overrides the container's image field
// directly, guaranteeing the release-pinned digest runs.
ControllerImage: "RELATED_IMAGE_ODH_MY_MODULE_OPERATOR_IMAGE",
RelatedImages:   []string{
    // Only operand images belong here
},
```

The distinction matters because `ControllerImage` mutates the `image:` field
on the rendered Deployment before it is applied to the cluster. If the
controller image is only in `RelatedImages`, it becomes an env var on the
container -- but the container itself still runs whatever image tag was in the
original manifest. In RHOAI, this means the module operator runs a stale or
non-pinned image instead of the release digest from the CSV.
...
Handler unit tests should verify both the controller image and related images
...
func TestImageHandling(t *testing.T) {
    ...
    // Controller image is set (image override, not env var)
    g.Expect(h.GetControllerImage()).Should(Equal("RELATED_IMAGE_ODH_MY_MODULE_OPERATOR_IMAGE"))

    // Operand images are declared (injected as env vars)
    g.Expect(h.GetRelatedImages()).Should(ConsistOf(
        "RELATED_IMAGE_ODH_OPERAND_A_IMAGE",
        "RELATED_IMAGE_ODH_OPERAND_B_IMAGE",
    ))

    // Controller image must NOT appear in RelatedImages
    g.Expect(h.GetRelatedImages()).ShouldNot(ContainElement(h.GetControllerImage()))
}
```

E2E tests should verify that the module operator Deployment on the
cluster has:
- The correct container `image:` field (from `ControllerImage`).
- The expected `RELATED_IMAGE_*` environment variables (from `RelatedImages`).
- The `APPLICATIONS_NAMESPACE` environment variable.
...
When registering a handler, provide a runlevel for DAG-based ordering:

```go
mr.Add(handler, mr.WithRunlevel(dag.RL(25)))
```

The runlevel determines where the module sits in the provisioning sequence.
See [Upgrade Ordering and Lifecycle](#upgrade-ordering-and-lifecycle) for
the full DAG model, runlevel assignments, and how to declare admin ack
gates.
...
## Upgrade Ordering and Lifecycle

The platform provisions components and modules in a deterministic order
using a directed acyclic graph (DAG) and coordinates upgrades through
admin acknowledgment gates and a platform version handshake. Module
developers need to understand three things: where their module sits in
the provisioning order, how to declare upgrade gates, and how to
complete the version handshake.

### DAG-based provisioning order

Every component and module participates in a single shared DAG. The
graph is resolved into ordered batches (runlevels) at reconciliation
time. The orchestrator advances through the batches sequentially — a
batch cannot begin until every entry in all prior batches reports
`Ready=True`. This applies across types: a component will not be
provisioned if a module in a prior batch is not ready, and vice versa.
...
A **runlevel** is an ordered provisioning tier. Every component and
module is assigned to exactly one runlevel. Lower-order runlevels are
provisioned first; all entries in a runlevel must be `Ready` before the
next runlevel begins.

| Expression   | Purpose                                                              |
|--------------|----------------------------------------------------------------------|
| `dag.RL(20)` | Core AI/ML components (dashboard, pipelines, workbenches, ray, etc.) |
| `dag.RL(25)` | Modules that need core components ready first                        |
| `dag.RL(31)` | Extension foundations (KServe, Kueue)                                |
| `dag.RL(32)` | Independent extensions (Feast, MLflow, OGX, Spark)                   |
| `dag.RL(33)` | KServe-dependent extensions (ModelController, ModelsAsService)        |
| `dag.RL(99)` | Fallback for unassigned entries (provisioned last)                    |

Pick any integer; lower values provision first. Use adjacent values
(e.g. 31, 32, 33) for fine-grained ordering within a range. Lower
values (0, 10) are available for future foundational services.

#### Choosing a runlevel for your module

| Scenario                               | Runlevel     |
|----------------------------------------|--------------|
| Infrastructure service (platform-level)| `dag.RL(0)`  |
| Module needing core components ready   | `dag.RL(25)` |
| Independent extension                  | `dag.RL(32)` |
| Extension that others depend on        | `dag.RL(31)` |
| Extension needing KServe ready first   | `dag.RL(33)` |

#### Registering your runlevel

Add an entry to the `moduleRunlevels` map in `cmd/main.go`:
...
The `registerModules()` function handles registration into the unified
provisioning registry. Cross-runlevel gating is automatic.

#### Readiness

An entry is considered ready when its CR has a `Ready=True` condition in
its status. For modules, the orchestrator also checks that
`observedGeneration` is not stale (the module controller has processed
the current spec). This is transparent to your integration — you only
need to ensure your controller writes `Ready=True` when provisioning is
complete.

#### Timeout policy

Every runlevel defaults to a 10-minute wall-clock timeout. If a
runlevel remains not-ready for longer than its timeout, the orchestrator
advances past it and continues processing later runlevels. While the
timed-out entries remain not-ready, a transient
`ProvisioningProgress=False` with reason `RunlevelTimeoutExceeded` is
recorded, but it may be overwritten as later runlevels complete. This
prevents a single stuck module from blocking the entire platform
indefinitely.

#### What happens when your module is disabled

Disabled entries are excluded from the DAG. Other entries at higher
runlevels are unaffected — they do not wait for a module that is not
enabled.

### Admin acknowledgment gates

Before any batch is processed, the orchestrator checks for
unacknowledged upgrade gates. If any gates for the current platform
version remain unacknowledged, provisioning is blocked entirely,
`ProvisioningProgress` is set to `False` with reason
`AdminAckRequired`, and the condition message lists the unacked keys.

Gates give module teams a mechanism to require administrator
intervention before an upgrade proceeds — for example, to confirm a
breaking API change has been reviewed, a data migration has been backed
up, or a deprecation notice has been acknowledged.

#### How gates work

All gate entries are merged into a single `odh-upgrade-acks` ConfigMap
in the operator namespace. Each key follows the pattern
`ack-<version>-<description>`. The value is either a human-readable
description (unacknowledged) or `"true"` (acknowledged). Entries
already set to `"true"` are never overwritten — prior acknowledgments
...
The operator watches the `odh-upgrade-acks` ConfigMap so provisioning
resumes automatically once all gates are acknowledged — no manual
reconcile trigger is needed.

#### Declaring gates from a module Helm chart

Include a ConfigMap in your module's Helm chart with the label
`platform.opendatahub.io/upgrade-gate: "true"`. The operator extracts
gate entries from rendered charts before deployment and merges them into
`odh-upgrade-acks`. The gate ConfigMap is intercepted and never applied
to the cluster as a standalone object.

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: mymodule-upgrade-gates
  labels:
    platform.opendatahub.io/upgrade-gate: "true"
data:
  ack-2.21.0-api-v1beta1-removal: >-
    The v1alpha1 API has been removed. Review the migration guide at
    https://docs.example.com/migrate-v1beta1 before acknowledging.
  ack-2.21.0-storage-schema-change: >-
    The storage schema has changed. Back up existing data before
    proceeding.
```

Each key must follow the `ack-<version>-<description>` pattern. The
value should be a human-readable message explaining what the admin
needs to acknowledge and any actions they should take before doing so.

#### Gate lifecycle

Gates are version-scoped. When the operator version advances, old gates
(from previous versions) are no longer checked. Only gates matching the
current operator version block provisioning. This means module teams
should include gates for the version in which a breaking change is
introduced — they are automatically inactive in subsequent versions.

#### Declaring gates from cluster ConfigMaps

Module operators can also create gate ConfigMaps directly on the
cluster with the label `platform.opendatahub.io/upgrade-gate: "true"`.
The platform discovers these at reconcile time and merges their entries
into `odh-upgrade-acks`. This is useful for gates that need to be
created dynamically (e.g., based on cluster state) rather than
statically in a Helm chart.

**Important:** There is no guarantee that a gate ConfigMap created by a
module operator will be discovered before the platform's first
provisioning pass. The platform reconciles on startup and may complete
one or more provisioning cycles before the module operator has started
and created its gate ConfigMap. For this reason, **Helm chart gates are
the preferred mechanism** — they are extracted from the rendered chart
before any resources are deployed, so they are always discovered before
provisioning begins. Use cluster ConfigMap gates only for dynamic,
runtime-determined gates where the blocking condition cannot be known
at chart render time.

### Platform version handshake

The platform version handshake is the coordination mechanism that
confirms a module controller has completed its upgrade to a new platform
version. Without this signal, the platform cannot distinguish between
"the module is running the new version" and "the module is still
processing the upgrade."

#### How it works

1. **Platform publishes its version per module.** The platform operator
   creates and maintains a `odh-<modulename>-config` ConfigMap for
   each enabled module in the applications namespace. This ConfigMap
   contains the `platformVersion` key. It is updated on every reconcile
   via Server-Side Apply, so the value always reflects the current
   platform operator version.
   ...
   If the module's Helm chart includes a ConfigMap with the same name
   (e.g., for controller configuration flags), the platform merges
   `platformVersion` into it — module-owned keys are preserved. See
   [Platform config ConfigMap](#platform-config-configmap).

2. **Module controller reads the version.** On each reconcile, the
   module controller should read the `platformVersion` from its
   `odh-<modulename>-config` ConfigMap. This tells the module what
   version of the platform contract it should be operating under.

3. **Module controller writes the version to its CR status.** After the
   module controller has completed all upgrade-related work for the new
   version (schema migrations, operand rollouts, feature flag updates,
   etc.), it writes the platform version into its CR
   `.status.releases` array as an entry with `name: "platform"`. This
   is the handshake signal — it tells the platform "I have fully
   processed the upgrade to version X."

4. **Platform reads the version from module CR status.** The platform
   orchestrator reads the `"platform"` entry from each module CR's
   `status.releases`. When its version matches the platform version,
   the upgrade handshake is complete for that module.

```text
Platform operator                              Module operator
┌──────────────┐                               ┌──────────────┐
│ writes       │  odh-<name>-config             │ reads        │
│ platform     ├──────────────────────────────►│ platform     │
│ version      │  .data.platformVersion        │ version      │
│              │                               │              │
│ reads module │  module CR .status            │ writes       │
│ CR status    │◄──────────────────────────────┤ version to   │
│ version      │  .releases[name="platform"]   │ .status      │
└──────────────┘                               └──────────────┘
```
...
| Data key            | `platformVersion`                                    |
| Namespace           | Applications namespace                               |
| Enforcement         | SSA — platform-managed keys reconciled on every cycle |

#### Module controller implementation

The `status.releases` entry with `name: "platform"` records the platform
version the module has successfully reconciled against. The module controller
must
only advance this field after it has fully completed any upgrade work
required by the new platform version. While upgrade work is in
progress, the field retains the previous platform version — this tells
the platform "I am still running against version X."

The module controller should:

1. **Read the platform version from `odh-<modulename>-config`** on
   each reconcile. Read `data.platformVersion` directly — it is a plain
   string, no YAML parsing needed. Watch the ConfigMap or mount it as
   a volume so version changes trigger a reconcile.

2. **Compare with the version in `status.releases[name="platform"]`.**
   If they match, no upgrade is in progress — proceed with normal
   reconciliation. If they differ, the platform has been upgraded and
   the module needs to handle the transition.

3. **Perform any version-specific upgrade logic.** This is
   module-specific — it might include schema migrations, operand image
   updates, configuration changes, or nothing at all if the new version
   requires no migration. While this work is in progress, do **not**
   update the `"platform"` release entry.

4. **Write the new platform version to `status.releases[name="platform"]`**
   only after all upgrade work is complete and the module is fully
   operational under the new version. This is the handshake signal.

5. **Set `Ready=True`** only after the version has been written and all
   operands are healthy under the new version.

```go
func (r *MyModuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
    // ... fetch module CR, reconcile operands ...

    platformVersion, err := r.getPlatformVersion(ctx)
    if err != nil {
        return ctrl.Result{}, fmt.Errorf("reading platform version: %w", err)
    }

    currentVersion := getPlatformRelease(instance).Version

    if currentVersion != platformVersion {
        // Platform has been upgraded — perform migration before
        // advancing the version in status.
        if err := r.handleUpgrade(ctx, instance, currentVersion, platformVersion); err != nil {
            // Upgrade still in progress or failed. Status retains
            // the old version so the platform knows we have not
            // finished the transition yet.
            return ctrl.Result{}, err
        }
    }

    // Upgrade complete (or no upgrade needed). Record the platform
    // version we have reconciled against.
    setPlatformRelease(instance, platformVersion)
...
func getPlatformRelease(instance *mymodulev1.MyModule) common.ComponentRelease {
...
func setPlatformRelease(instance *mymodulev1.MyModule, version string) {
    for i, r := range instance.Status.Releases {
        if r.Name == "platform" {
            instance.Status.Releases[i].Version = version
            return
        }
    }
...
The version handshake solves a specific problem: during a platform
upgrade, the platform operator is upgraded first (new binary, new
images), and then it reconciles module CRs with updated spec. But the
module controller binary may also have been upgraded (via the
`ControllerImage` injection), and it may need to perform migration
steps before it can report healthy.

Without the handshake, the platform would see `Ready=True` and assume
the upgrade is complete, even if the module is still running old logic
or has not processed migration steps. The version field makes the
upgrade explicit and observable.

The handshake also enables the platform to:

- **Detect stalled upgrades.** If a module's status version does not
  advance to the platform version within the runlevel timeout, the
  platform knows the module is stuck.
- **Gate downstream modules.** Modules in later runlevels are not
  provisioned until earlier modules have completed their upgrade
  (version matches + `Ready=True`).
- **Provide observability.** Administrators can inspect module CR
  status to see exactly which version each module is running and
  whether any are lagging behind the platform.

### Status conditions for upgrade lifecycle

The platform writes to dedicated provisioning progress conditions,
separate from component/module health rollups:
...
`ProvisioningProgress` is `True` when all runlevels have been processed,
and `False` when:

- The orchestrator is blocked on a runlevel (reason `AwaitingReadiness`)
- A runlevel has timed out (reason `RunlevelTimeoutExceeded`) — this is
  transient; the orchestrator advances past the stuck entries and the
  reason may be overwritten as later runlevels complete
- Admin ack gates are unacknowledged (reason `AdminAckRequired`)

Module teams do not need to write to `ProvisioningProgress` — the
platform orchestrator manages it. Module teams only need to write
accurate `Ready`, `Degraded`, and `ProvisioningSucceeded` conditions on
their own CR status.
