...
func (w *LLMISVCConnectionWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case admissionv1.Create, admissionv1.Update:
		...
		allowedTypes := []string{
			webhookutils.ConnectionTypeURI.String(), // TODO: do we need this?
			webhookutils.ConnectionTypeS3.String(),
			webhookutils.ConnectionTypeOCI.String(),
			webhookutils.ConnectionTypeHF.String(),
		}
		...
		case webhookutils.ConnectionActionInject:
			...
			if !isDryRun {
				...
				if err := webhookutils.CreateServiceAccount(ctx, w.Webhook.Client, secretName, req.Namespace); err != nil {
...
func (w *LLMISVCConnectionWebhook) injectModelUri(ctx context.Context, obj *unstructured.Unstructured, secretName, namespace string) error {
	...
	if err := w.Webhook.Client.Get(ctx, types.NamespacedName{Name: secretName, Namespace: namespace}, secret); err != nil {
