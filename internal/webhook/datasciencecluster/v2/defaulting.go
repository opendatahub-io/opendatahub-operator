...
// Package v2 provides admission webhook logic for defaulting DataScienceCluster v2 resources.
...
func (d *Defaulter) SetupWithManager(mgr ctrl.Manager) error {
	...
	mgr.GetWebhookServer().Register("/mutate-datasciencecluster-v2", mutateWebhook)
	// No error to return currently, but return nil for future extensibility
	return nil
}

// Default sets default values on the provided DataScienceCluster v2 object.
//
// Parameters:
...
func (d *Defaulter) Default(ctx context.Context, obj runtime.Object) error {
	...
	if !isDSC {
		...
		err := fmt.Errorf("expected DataScienceCluster v2 but got a different type: %T", obj)
...
// applyDefaults applies default values to the DataScienceCluster v2 resource in-place.
