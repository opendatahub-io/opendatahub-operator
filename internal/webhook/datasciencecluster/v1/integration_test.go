...
func TestDataScienceClusterV1_Integration(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			name: "CEL: TrainingOperator (KFTO v1) explicit Removed->Managed re-enablement blocked on update via v1 API",
			setup: func(ns string) []client.Object {
				return nil
			},
			test: func(g Gomega, ctx context.Context, k8sClient client.Client, ns string) {
				dsc := envtestutil.NewDSCV1("dsc-kfto-cel-v1-removed", func(d *dscv1.DataScienceCluster) {
				...
				key := types.NamespacedName{Name: "dsc-kfto-cel-v1-removed", Namespace: ns}
				fetched := &dscv1.DataScienceCluster{}
				g.Expect(k8sClient.Get(ctx, key, fetched)).To(Succeed())
