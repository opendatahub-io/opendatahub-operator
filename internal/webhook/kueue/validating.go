...
func (v *Validator) performLabelValidation(ctx context.Context, req *admission.Request) admission.Response {
	...
	kueueManagedNamespace, err := v.isNamespaceManagedByKueue(ctx, namespace)
	if err != nil {
	...
	kueueEnabled, err := v.isKueueEnabledInDSC(ctx)
...
func (v *Validator) isKueueEnabledInDSC(ctx context.Context) (bool, error) {
	dsc, err := cluster.GetDSC(ctx, v.Client)
	if err != nil {
		return false, err
	}
