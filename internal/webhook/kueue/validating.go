...
// - batch/v1: jobs
// - kubeflow.org/v1: mpijobs, jaxjobs, paddlejobs, pytorchjobs, tfjobs, xgboostjobs, notebooks
...
//+kubebuilder:webhook:path=/validate-kueue,mutating=false,failurePolicy=fail,sideEffects=None,groups=kubeflow.org,resources=mpijobs;jaxjobs;paddlejobs;pytorchjobs;tfjobs;xgboostjobs;notebooks,verbs=create;update,versions=v1,name=kubeflow-kueuelabels-validator.opendatahub.io,admissionReviewVersions=v1
...
func performLabelValidation(ctx context.Context, cli client.Reader, req *admission.Request) admission.Response {
	...
	labelErr := validateKueueLabels(meta.GetLabels())
	hasKueueLabels := labelErr == nil

	switch {
	case kueueEnabled && kueueManagedNamespace && !hasKueueLabels:
		// Kueue is enabled, namespace is labeled for Kueue, but no Kueue labels found
		return admission.Denied(fmt.Sprintf("Kueue label validation failed: %v", labelErr))
	case hasKueueLabels && !kueueEnabled:
		// Kueue labels found but Kueue is not enabled in the DSC
		return admission.Denied(fmt.Sprintf("Kueue is not enabled but workload %q is using Kueue label", req.Kind.Kind))
	case hasKueueLabels && !kueueManagedNamespace:
	...
	default:
		// Kueue is not enabled or namespace labeling, and no workload label present
		return admission.Allowed("Kueue label validation skipped (Kueue is not enabled or namespace labeling, and no workload label present)")
	}
}
