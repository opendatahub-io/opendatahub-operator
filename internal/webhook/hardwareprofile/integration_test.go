...
// Container path constants for different workload types.
var (
	notebookContainersPath           = []string{"spec", "template", "spec", "containers"}
	notebookNodeSelectorPath         = []string{"spec", "template", "spec", "nodeSelector"}
	notebookTolerationsPath          = []string{"spec", "template", "spec", "tolerations"}
	inferenceServiceContainersPath   = []string{"spec", "predictor", "podSpec", "containers"}
	inferenceServiceNodeSelectorPath = []string{"spec", "predictor", "podSpec", "nodeSelector"}
	inferenceServiceTolerationsPath  = []string{"spec", "predictor", "podSpec", "tolerations"}
)
		...
		},
		{
			...
			test: func(g Gomega, ctx context.Context, k8sClient client.Client, ns string) {
				testHardwareProfileWithNodeSchedulingForWorkload(g, ctx, k8sClient, ns,
					...
					notebookNodeSelectorPath, notebookTolerationsPath)
			},
		},
		{
		...
		// InferenceService test cases
		{
			...
			test: func(g Gomega, ctx context.Context, k8sClient client.Client, ns string) {
				testNoHardwareProfileAnnotationForWorkload(g, ctx, k8sClient,
					...
					inferenceServiceContainersPath)
			},
		},
		{
		...
		},
		{
			...
			test: func(g Gomega, ctx context.Context, k8sClient client.Client, ns string) {
				testHardwareProfileWithNodeSchedulingForWorkload(g, ctx, k8sClient, ns,
					...
					inferenceServiceNodeSelectorPath, inferenceServiceTolerationsPath)
			},
		},
		{
