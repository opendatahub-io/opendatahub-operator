...
func TestRegisterWebhooks_NilManager(t *testing.T) {
	...
	// Test with nil manager - should return error
	err := monitoring.RegisterWebhooks(nil)

	g.Expect(err).Should(HaveOccurred(), "RegisterWebhooks(nil) should return error")
	g.Expect(err.Error()).Should(ContainSubstring("manager cannot be nil"))
}
