...
func (i *Injector) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	// Check if client is properly injected
	if i.Client == nil {
...
func (i *Injector) performMonitoringInjection(ctx context.Context, req *admission.Request, obj *unstructured.Unstructured) admission.Response {
	...
	if isMonitoredNamespace, exists := namespaceLabels["opendatahub.io/monitoring"]; exists {
		if isMonitoredNamespace == "true" {
			...
			// Check if Monitoring CR exists (verifies monitoring is actually enabled)
			monitoring := &unstructured.Unstructured{}
			monitoring.SetGroupVersionKind(gvk.Monitoring)
			monitoringName := types.NamespacedName{Name: "default-monitoring"} // cluster-scoped singleton

			if err := i.Client.Get(ctx, monitoringName, monitoring); err != nil {
				if k8serr.IsNotFound(err) {
				...
				log.Error(err, "Failed to check Monitoring CR existence",
					"resource", obj.GetName(),
					"namespace", resourceNamespace)
				return admission.Errored(http.StatusInternalServerError,
					fmt.Errorf("failed to verify monitoring state: %w", err))
			}

			log.V(1).Info("Monitoring CR found - proceeding with label injection",
				"resource", obj.GetName(),
				"namespace", resourceNamespace)
