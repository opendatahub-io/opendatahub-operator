...
// NewReconciler sets up the EKSKubernetesEngine controller and registers it with the manager.
func NewReconciler(ctx context.Context, mgr ctrl.Manager, cfg *operatorconfig.CloudManagerConfig) error {
	resourceID := labels.NormalizePartOfValue(ccmv1alpha1.EKSKubernetesEngineKind)
	bootstrapConfig := certmanager.DefaultBootstrapConfig(certmanager.WithOperatorCert(cfg.RhaiOperatorNamespace))

	_, err := reconciler.ReconcilerFor(mgr, &ccmv1alpha1.EKSKubernetesEngine{}).
		WithDynamicOwnership().
		Watches(
			&extv1.CustomResourceDefinition{},
			reconciler.WithEventHandler(handlers.ToNamed(ccmv1alpha1.EKSKubernetesEngineInstanceName)),
			reconciler.WithPredicates(resources.CreatedOrUpdatedOrDeletedNamed(common.ServiceMonitorCRDName)),
		).
		WithAction(initialize).
		ComposeWith(certmanager.Bootstrap[*ccmv1alpha1.EKSKubernetesEngine](
			ccmv1alpha1.EKSKubernetesEngineInstanceName,
			bootstrapConfig,
		)).
