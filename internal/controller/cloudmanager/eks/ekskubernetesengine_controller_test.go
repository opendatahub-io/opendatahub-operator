...
import (
	...
	"github.com/opendatahub-io/opendatahub-operator/v2/internal/controller/cloudmanager/eks"
...
func hasCertManagerDeployments(wt *testf.WithT) bool {
	return ccmtest.HasInfraDeployments(wt, nsCertManagerOperator, ccmv1alpha1.EKSKubernetesEngineKind)
}

var eksCfg = ccmtest.ControllerTestConfig{
	CRDSubdir:     "eks",
	NewReconciler: eks.NewReconciler,
	NewCR: func(deps ccmcommon.Dependencies) client.Object {
		return &ccmv1alpha1.EKSKubernetesEngine{
			ObjectMeta: metav1.ObjectMeta{
				Name: ccmv1alpha1.EKSKubernetesEngineInstanceName,
			},
			Spec: ccmv1alpha1.EKSKubernetesEngineSpec{
				Dependencies: deps,
			},
		}
	},
	InstanceName: ccmv1alpha1.EKSKubernetesEngineInstanceName,
	InfraLabel:   "ekskubernetesengine",
	GVK:          gvk.EKSKubernetesEngine,
}

func TestEKSKubernetesEngine(t *testing.T) {
	ccmtest.RequireCharts(t)

	t.Run("namespaces do not have owner references", func(t *testing.T) {
		wt := tc.NewWithT(t)

		ccmtest.CreateCR(t, wt, eksCfg, ccmcommon.Dependencies{
	...
	t.Run("creates PKI bootstrap resources when cert-manager is installed", func(t *testing.T) {
		...
		et, wtC := ccmtest.StartIsolatedController(t, ctx, eksCfg)
		...
		ccmtest.CreateCR(t, wtC, eksCfg, ccmcommon.Dependencies{
			CertManager:  ccmcommon.CertManagerDependency{ManagementPolicy: ccmcommon.Managed},
			LWS:          ccmcommon.LWSDependency{ManagementPolicy: ccmcommon.Managed},
			SailOperator: ccmcommon.SailOperatorDependency{ManagementPolicy: ccmcommon.Managed},
		})

		nn := types.NamespacedName{Name: ccmv1alpha1.EKSKubernetesEngineInstanceName}
		...
		wtC.Get(gvk.EKSKubernetesEngine, nn).Eventually().Should(
			jq.Match(`.status.conditions[] | select(.type == "DependenciesAvailable") | .status == "True"`),
		)
	})
}

// TestEKSKubernetesEngineGC tests garbage collection behavior. All subtests share a
...
func TestEKSKubernetesEngineGC(t *testing.T) {
	ccmtest.RequireCharts(t)

	ctx, cancel := context.WithCancel(context.Background())
	et, wt := ccmtest.StartIsolatedController(t, ctx, eksCfg)
	t.Cleanup(cancel)
	...
	t.Run("deletes resources of dependency that transitions to Unmanaged", func(t *testing.T) {
		...
		ake := &ccmv1alpha1.EKSKubernetesEngine{}
		wt.Expect(wt.Client().Get(wt.Context(),
			types.NamespacedName{Name: ccmv1alpha1.EKSKubernetesEngineInstanceName}, ake)).To(Succeed())
		...
		wt.Eventually(func() bool {
			list, err := ccmtest.ListInfraDeployments(wt, nsCertManagerOperator, ccmv1alpha1.EKSKubernetesEngineKind)
		...
		staleCM := &corev1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					labels.InfrastructurePartOf: "ekskubernetesengine",
				},
				...
				OwnerReferences: []metav1.OwnerReference{{
					APIVersion: gvk.EKSKubernetesEngine.GroupVersion().String(),
					Kind:       gvk.EKSKubernetesEngine.Kind,
					Name:       ake.GetName(),
					UID:        ake.GetUID(),
				}},
			},
...
// TestEKSKubernetesEngineWithoutCertManager tests cert-manager CRD absence and dynamic
// registration. Each sub-test uses an isolated envtest to start with zero cert-manager CRDs.
func TestEKSKubernetesEngineWithoutCertManager(t *testing.T) {
	ccmtest.RequireCharts(t)
	...
	t.Run("reports DependenciesAvailable=False and Ready=False when cert-manager CRDs absent", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		_, wtC := ccmtest.StartIsolatedController(t, ctx, eksCfg)
		t.Cleanup(cancel) // stop the manager before the test environment (registered after et.Stop, so it runs first)
