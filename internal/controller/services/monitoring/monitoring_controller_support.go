...
func addTracesTemplateData(templateData map[string]any, traces *serviceApi.Traces, namespace string) error {
	...
	// Add multitenancy configuration (defaults to enabled with OpenShift mode)
	// This can be made configurable via API in the future if needed for non-OpenShift deployments
	templateData["MultitenancyEnabled"] = true
	templateData["TempoMultitenancyMode"] = "openshift"
	...
	if tlsEnabled {
		if traces.TLS != nil {
			...
			// TLS explicitly enabled but no certificates provided - let Tempo operator use OpenShift serving certificate service
			// When certName is not provided on OpenShift, the operator uses the serving certificate service
