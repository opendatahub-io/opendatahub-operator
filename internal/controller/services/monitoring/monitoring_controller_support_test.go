...
func validateConditions(t *testing.T, rr *odhtypes.ReconciliationRequest, tt monitoringIntegrationTestCase) {
	...
	if tt.expectedTQConditionStatus == "" {
		assert.Nil(t, thanosCondition,
			"ThanosQuerier condition should not be set when no metrics configured")
	} else {
		assert.NotNil(t, thanosCondition,
			"ThanosQuerier condition should be set when metrics configured")
...
func TestMonitoringStackThanosQuerierIntegration(t *testing.T) {
	...
	tests := []monitoringIntegrationTestCase{
		{
			...
			expectedMSTemplates:       2, // MonitoringStack + PrometheusRoute
		...
		{
			name:                      "Only MonitoringStack CRD available with metrics - MS deployed, TQ condition false",
			hasMetricsConfig:          true,
			hasMonitoringStackCRD:     true,
			hasThanosQuerierCRD:       false,
			expectedMSConditionStatus: "True",
			expectedTQConditionStatus: "False",
			expectedMSTemplates:       2,
			expectedTQTemplates:       0,
			description:               "When only MonitoringStack CRD is available, MonitoringStack should still be deployed",
		},
		{
			name:                      "Only ThanosQuerier CRD available with metrics - MS condition false, TQ deployed",
			hasMetricsConfig:          true,
			...
			expectedTQTemplates:       2,
			description:               "When only ThanosQuerier CRD is available, ThanosQuerier should still be deployed",
		},
		{
		...
		{
			name:                      "No metrics configuration - MS condition false, TQ not called",
			hasMetricsConfig:          false,
			hasMonitoringStackCRD:     true,
			hasThanosQuerierCRD:       true,
			expectedMSConditionStatus: "False",
			expectedTQConditionStatus: "", // ThanosQuerier should not be called when no metrics
			expectedMSTemplates:       0,
			expectedTQTemplates:       0,
			description:               "When no metrics configured, MonitoringStack should not be deployed and ThanosQuerier should not be called",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = deployMonitoringStack(ctx, rr)
			require.NoError(t, err, "deployMonitoringStack should not return error")
