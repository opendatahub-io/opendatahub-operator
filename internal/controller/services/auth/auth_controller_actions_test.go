...
func TestInitialize(t *testing.T) {
	...
	g.Expect(rr.Templates[4].Path).To(Equal(DataScienceMetricsAllowedClusterRoleTemplate))
}
...
func TestManagePermissionsWithMetricsGroups(t *testing.T) {
	...
	auth := &serviceApi.Auth{
		...
		Spec: serviceApi.AuthSpec{
			...
			MetricsAllowedGroups: []string{"metrics-user1", "metrics-user2"},
		},
	}
	...
	// Verify resources were created (5 total: 2 RoleBindings + 3 ClusterRoleBindings)
	...
	// Verify that metrics-specific cluster roles are created
	g.Expect(clusterRoleNames).To(ContainElement("data-science-metrics-admin"), "Should create metrics admin cluster role")
	g.Expect(clusterRoleNames).To(ContainElement("data-science-metrics-allowed"), "Should create metrics allowed cluster role")
	...
	g.Expect(roleNames).To(ContainElement("allowedgroup-role"), "Should create allowed group role")
}
...
func TestManagePermissionsWithEmptyMetricsGroups(t *testing.T) {
	...
	auth := &serviceApi.Auth{
		...
		Spec: serviceApi.AuthSpec{
			...
			MetricsAllowedGroups: []string{}, // Empty
		},
	}
