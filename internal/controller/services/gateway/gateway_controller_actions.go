...
func createComponentResources(ctx context.Context, rr *odhtypes.ReconciliationRequest) error {
	l := logf.FromContext(ctx).WithName("createComponentResources")
	l.V(1).Info("Creating Auth related resources for components")
	...
	componentChecks := map[string]struct {
		...
		instanceName          string
	}{ // TODO: extend this list for other components. we should not need one for workbenches but let NBC create one in each DSProject.
		"dashboard": {
			shouldCreateHttproute: func() bool {
