...
func createOAuthClient(ctx context.Context, rr *odhtypes.ReconciliationRequest, gatewayConfig *serviceApi.GatewayConfig) error {
	...
	clientSecretBytes, exists := authSecret.Data[EnvClientSecret]
	if !exists {
		return fmt.Errorf("%s not found in secret %s/%s", EnvClientSecret, GatewayNamespace, KubeAuthProxySecretsName)
	}
...
func getSecretValues(
	...
	if secretErr == nil {
		...
		clientIDBytes, hasClientID := existingSecret.Data[EnvClientID]
...
func calculateAuthConfigHash(authSecret *corev1.Secret) string {
	clientID := string(authSecret.Data[EnvClientID])
	clientSecret := string(authSecret.Data[EnvClientSecret])
	cookieSecret := string(authSecret.Data[EnvCookieSecret])
