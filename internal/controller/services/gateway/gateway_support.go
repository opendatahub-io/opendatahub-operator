...
const (
	...
	DefaultKubeAuthProxyImage = "quay.io/jtanner/kube-auth-proxy@sha256:434580fd42d73727d62566ff6d8336219a31b322798b48096ed167daaec42f07"
)

// GetKubeAuthProxyImage returns the KubeAuthProxy image to use.
// It first checks the ABC environment variable, and falls back to the default image if not set.
...
func GetFQDN(ctx context.Context, cli client.Client, gatewayConfig *serviceApi.GatewayConfig) (string, error) {
	fqdn := gatewayConfig.Spec.IngressGateway.Domain
	if fqdn == "" {
		clusterDomain, err := cluster.GetDomain(ctx, cli)
		if err != nil {
			return "", fmt.Errorf("failed to get cluster domain: %w", err)
		}
		fqdn = fmt.Sprintf("%s.%s", DefaultGatewayName, clusterDomain)
	}
	return fqdn, nil
}
