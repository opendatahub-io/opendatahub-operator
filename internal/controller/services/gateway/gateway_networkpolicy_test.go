...
func TestGetNetworkPolicyTemplateData(t *testing.T) {
	...
	expectedKeys := []string{"Name", "Namespace", "GatewayName", "HTTPSPort", "MetricsPort"}
	for _, key := range expectedKeys {
		g.Expect(data).To(HaveKey(key), "should contain key: %s", key)
	}

	// Verify values match expected constants
	g.Expect(data["Name"]).To(Equal(KubeAuthProxyName), "Name should match KubeAuthProxyName")
	g.Expect(data["Namespace"]).To(Equal(GatewayNamespace), "Namespace should match GatewayNamespace")
...
func TestGetNetworkPolicyTemplateDataWithNilRequest(t *testing.T) {
	...
	g.Expect(data).NotTo(BeNil(), "should return non-nil data map")
	g.Expect(data).To(HaveKey("Name"), "should contain Name key")
}
