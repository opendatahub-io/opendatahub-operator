...
func testDSCContext() *modules.DSCContext {
	return &modules.DSCContext{DSC: &dscv2.DataScienceCluster{}}
}

func testDSCIContext() *modules.DSCContext {
...
func loadCRDFromArtifacts(t *testing.T, handler modules.ModuleHandler, manifestsRoot, chartsRoot string) *apiextensionsv1.CustomResourceDefinition {
	...
	searchPaths := make([]string, 0, 3+2*len(manifests.Manifests)+len(manifests.HelmCharts))
...
func TestModuleCRSchemaCompliance(t *testing.T) {
	...
	dsciCtx := testDSCIContext()
	platformModules := testEnabledPlatformModules()
	...
	for _, handler := range handlers {
		...
		if !handler.IsEnabled(platformModules) {
			continue
		}
		...
		t.Run(handler.GetName(), func(t *testing.T) {
			...
			cr, err := handler.BuildModuleCR(context.Background(), nil, handlerDSCContext, crConfig)
			g.Expect(err).ShouldNot(HaveOccurred(), "BuildModuleCR failed for %s", handler.GetName())
			g.Expect(cr).ShouldNot(BeNil(), "BuildModuleCR returned nil for %s", handler.GetName())
