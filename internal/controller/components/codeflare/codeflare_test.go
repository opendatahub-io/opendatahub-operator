...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.CodeFlareComponentName))
}
...
func TestNewCRObject(t *testing.T) {
	...
	g := NewWithT(t)
	dsc := createDSCWithCodeFlare(operatorv1.Managed)

	cr := handler.NewCRObject(dsc)
	g.Expect(cr).ShouldNot(BeNil())
	g.Expect(cr).Should(BeAssignableToTypeOf(&componentApi.CodeFlare{}))

	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.CodeFlareInstanceName),
		jq.Match(`.kind == "%s"`, componentApi.CodeFlareKind),
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			dsc := createDSCWithCodeFlare(tt.state)
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle enabled component with ready CodeFlare CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithCodeFlare(operatorv1.Managed)
		codeflare := createCodeFlareCR(true)
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			...
			jq.Match(`.status.components.codeflare.managementState == "%s"`, operatorv1.Managed),
	...
	t.Run("should handle enabled component with not ready CodeFlare CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithCodeFlare(operatorv1.Managed)
		codeflare := createCodeFlareCR(false)
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithCodeFlare(operatorv1.Removed)
...
func createDSCWithCodeFlare(managementState operatorv1.ManagementState) *dscv2.DataScienceCluster {
	dsc := dscv2.DataScienceCluster{}
	dsc.SetGroupVersionKind(gvk.DataScienceCluster)
	dsc.SetName("test-dsc")

	dsc.Spec.Components.CodeFlare.ManagementState = managementState
	dsc.Status.InstalledComponents = make(map[string]bool)

	return &dsc
}

func createCodeFlareCR(ready bool) *componentApi.CodeFlare {
	c := componentApi.CodeFlare{}
	c.SetGroupVersionKind(gvk.CodeFlare)
	c.SetName(componentApi.CodeFlareInstanceName)
