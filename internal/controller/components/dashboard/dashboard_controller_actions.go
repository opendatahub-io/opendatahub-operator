...
// updateDashboardConfigMapHash annotates the dashboard deployment with the ConfigMap hash
// to trigger automatic pod restart when the ConfigMap changes.
// This follows the same pattern as customizeKserveConfigMap in kserve.
func updateDashboardConfigMapHash(ctx context.Context, rr *odhtypes.ReconciliationRequest) error {
	platform := rr.Release.Name

	// Get ConfigMap name based on platform
	configMapName := getConfigMapName(platform)

	// Find the ConfigMap in rendered resources (generated by kustomize from params.env)
	paramsConfigMap := corev1.ConfigMap{}
	_, err := getIndexedResource(rr.Resources, &paramsConfigMap, gvk.ConfigMap, configMapName)
	if err != nil {
		// ConfigMap might not exist yet, skip hash update
		return nil //nolint:nilerr // ConfigMap not found is expected in some cases, skip hash update
	}

	// Compute hash of the ConfigMap (following kserve pattern)
	configHash, err := hashConfigMap(&paramsConfigMap)
	if err != nil {
		return fmt.Errorf("failed to hash ConfigMap %s: %w", configMapName, err)
	}

	// Get deployment name based on platform
	deploymentName := getDeploymentName(platform)

	dashboardDeployment := appsv1.Deployment{}
	deployidx, err := getIndexedResource(rr.Resources, &dashboardDeployment, gvk.Deployment, deploymentName)
	if err != nil {
		// Deployment might not exist yet, skip hash update
		return nil //nolint:nilerr // Deployment not found is expected in some cases, skip hash update
	}

	// Ensure annotations map exists (following kserve pattern)
	if dashboardDeployment.Spec.Template.Annotations == nil {
		dashboardDeployment.Spec.Template.Annotations = make(map[string]string)
	}

	// Add ConfigMap hash annotation to trigger pod restart when ConfigMap changes
	// This follows the same pattern as kserve: labels.ODHAppPrefix+"/KserveConfigHash"
	hashAnnotationKey := labels.ODHAppPrefix + "/DashboardParamsHash"
	dashboardDeployment.Spec.Template.Annotations[hashAnnotationKey] = configHash

	// Replace the deployment in resources (following kserve pattern)
	if err = replaceResourceAtIndex(rr.Resources, deployidx, &dashboardDeployment); err != nil {
		return fmt.Errorf("failed to update deployment with ConfigMap hash: %w", err)
	}
