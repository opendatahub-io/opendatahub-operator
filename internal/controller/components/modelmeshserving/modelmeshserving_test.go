...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.ModelMeshServingComponentName))
}
...
func TestNewCRObject(t *testing.T) {
	...
	g := NewWithT(t)
	dsc := createDSCWithModelMeshServing(operatorv1.Managed)

	cr := handler.NewCRObject(dsc)
	g.Expect(cr).ShouldNot(BeNil())
	g.Expect(cr).Should(BeAssignableToTypeOf(&componentApi.ModelMeshServing{}))

	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.ModelMeshServingInstanceName),
		jq.Match(`.kind == "%s"`, componentApi.ModelMeshServingKind),
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			dsc := createDSCWithModelMeshServing(tt.state)
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle enabled component with ready ModelMeshServing CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithModelMeshServing(operatorv1.Managed)
		modelmeshserving := createModelMeshServingCR(true)

		cli, err := fakeclient.New(fakeclient.WithObjects(dsc, modelmeshserving))
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			...
			jq.Match(`.status.components.modelmeshserving.managementState == "%s"`, operatorv1.Managed),
	...
	t.Run("should handle enabled component with not ready ModelMeshServing CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithModelMeshServing(operatorv1.Managed)
		modelmeshserving := createModelMeshServingCR(false)
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithModelMeshServing(operatorv1.Removed)
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			...
			jq.Match(`.status.components.modelmeshserving.managementState == "%s"`, operatorv1.Removed),
			jq.Match(`.status.conditions[] | select(.type == "%s") | .status == "%s"`, ReadyConditionType, metav1.ConditionFalse),
	...
	t.Run("should handle empty management state as Removed", func(t *testing.T) {
		...
		dsc := createDSCWithModelMeshServing("")
...
func createDSCWithModelMeshServing(managementState operatorv1.ManagementState) *dscv1.DataScienceCluster {
	...
	dsc.Spec.Components.ModelMeshServing.ManagementState = managementState
	dsc.Status.InstalledComponents = make(map[string]bool)

	return &dsc
}

func createModelMeshServingCR(ready bool) *componentApi.ModelMeshServing {
	c := componentApi.ModelMeshServing{}
	c.SetGroupVersionKind(gvk.ModelMeshServing)
	c.SetName(componentApi.ModelMeshServingInstanceName)
