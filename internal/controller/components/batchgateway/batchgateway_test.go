...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.BatchGatewayComponentName))
}
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			dsc := createDSCWithBatchGateway(tc.state)
			if tc.shouldEnabled {
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should return ConditionFalse when component CR has deletionTimestamp", func(t *testing.T) {
		...
		dsc := createDSCWithBatchGateway(operatorv1.Managed)
		cr := createBatchGatewayCR(true)
		now := metav1.Now()
	...
	t.Run("should handle enabled component with ready BatchGateway CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.batchgateway.managementState == "%s"`, operatorv1.Managed),
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithBatchGateway(operatorv1.Removed)
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.batchgateway.managementState == "%s"`, operatorv1.Removed),
			jq.Match(`.status.conditions[] | select(.type == "%s") | .status == "%s"`, ReadyConditionType, metav1.ConditionFalse),
...
func createDSCWithBatchGateway(managementState operatorv1.ManagementState) *dscv2.DataScienceCluster {
	dsc := dscv2.DataScienceCluster{}
	dsc.SetGroupVersionKind(gvk.DataScienceCluster)
	dsc.SetName("test-dsc")

	dsc.Spec.Components.BatchGateway.ManagementState = managementState

	return &dsc
}

func createBatchGatewayCR(ready bool) *componentApi.BatchGateway {
	c := componentApi.BatchGateway{}
	c.SetGroupVersionKind(gvk.BatchGateway)
	c.SetName(componentApi.BatchGatewayInstanceName)
