...
2. [api-docs](https://github.com/opendatahub-io/opendatahub-operator/blob/main/.github/workflows/test-required-files-updated.yaml): Ensure the api-docs are updated when making changes to operator apis. If it fails, run `make generate manifests api-docs` to resolve errors. After GatewayConfig API changes, also run `make generate-xks-gateway-crd` and sync the file into odh-gitops (`charts/dependencies/xks-gateway/scripts/sync-gatewayconfig-crd.sh`). See [config/crd/xks/README.md](config/crd/xks/README.md) and [docs/aks-xks-gateway-testing.md](docs/aks-xks-gateway-testing.md).
