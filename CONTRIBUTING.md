...
6. **Sync Changes to Downstream**: After a PR is merged into the `main` branch, changes will be automatically synced with the downstream `rhoai` branch by the [sync-main-to-rhoai workflow](.github/workflows/sync-main-to-rhoai.yaml), which runs every 2 hours. You can verify the sync by checking the [workflow runs](https://github.com/opendatahub-io/opendatahub-operator/actions/workflows/sync-main-to-rhoai.yaml). During code freeze periods, see the [manual sync procedure](./docs/release-workflow-guide.md#rhoai-code-freeze-and-manual-sync-workflow).
7. **Verify Sync (Optional)**: To confirm your changes synced to `rhoai`:
     - Check the [sync workflow runs](https://github.com/opendatahub-io/opendatahub-operator/actions/workflows/sync-main-to-rhoai.yaml)
     - Look for your commit SHA in the `rhoai` branch
