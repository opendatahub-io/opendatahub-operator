...
2. [api-docs](https://github.com/opendatahub-io/opendatahub-operator/blob/main/.github/workflows/test-required-files-updated.yaml): Ensure generated files are updated when changing operator APIs. If CI fails, run `make generate manifests-all api-docs`. After `GatewayConfig` API changes (`api/services/v1alpha1/gateway_types.go`), also sync the CRD into [odh-gitops](https://github.com/opendatahub-io/odh-gitops): run `make manifests` in this repo, then in odh-gitops run `./charts/dependencies/xks-gateway/scripts/sync-gatewayconfig-crd.sh /path/to/opendatahub-operator` and `make verify-xks-gateway-crd OPERATOR_DIR=/path/to/opendatahub-operator`. See `charts/dependencies/xks-gateway/README.md` in odh-gitops.
