...
import (
	...
	templateutils "github.com/opendatahub-io/operator-actions-framework/utils/template"
...
// WithNamespaceFn sets the function used to determine the application namespace.
// The namespace is added to the template data under the AppNamespaceKey.
...
// WithFuncMap sets custom template functions to use during rendering.
// These functions are merged with the default functions.
func WithFuncMap(fm gt.FuncMap) ActionOpts {
	return func(action *Action) {
		maps.Copy(action.funcMap, fm)
	}
}
...
func (a *Action) render(ctx context.Context, rr *types.ReconciliationRequest) (resources.UnstructuredList, error) {
	...
	for i := range rr.Templates {
		tmpl, err := gt.New("").Option("missingkey=error").Funcs(a.funcMap).ParseFS(rr.Templates[i].FS, rr.Templates[i].Path)
		if err != nil {
