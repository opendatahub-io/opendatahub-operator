...
This all happens in a single reconciliation pass, not as a delayed or separate cleanup job. So when a reconciliation renders resources, old stale routes are deleted in that pass — there is no separate cleanup job.
...
| Operator uninstalled without deleting CRs first | Routes remain until the CRs are deleted or cleaned up manually |
