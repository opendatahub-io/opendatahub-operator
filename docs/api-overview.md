...
| `autoCreateQueues` _boolean_ | AutoCreateQueues controls whether the operator automatically creates default<br />ClusterQueue and LocalQueue resources in managed namespaces. When false (the default),<br />the operator skips queue creation entirely, allowing administrators to manage queues<br />via GitOps or other external tooling. HardwareProfiles of type "Queue" continue to<br />reference externally-managed LocalQueues without change. | false |  |
