# GPU Metrics Collection in RHOAI

## Overview

This document describes the GPU metrics collection feature implemented in Red Hat OpenShift AI (RHOAI) via the OpenTelemetry (otel) collector. This feature allows independent collection of GPU accelerator metrics into the RHOAI Prometheus instance when monitoring is enabled and properly configured.

## Architecture

The GPU metrics collection integrates with the existing RHOAI monitoring infrastructure:
...
GPU metrics collection is enabled when:

1. `.spec.monitoring.managementState` is set to `managed`
2. `.spec.monitoring.metrics` configuration is present

When either condition is not met, GPU metrics are not scraped.

## Implementation Details

### Template Data Configuration

The monitoring controller support (`monitoring_controller_support.go`) adds a `GPUMetrics` flag to the template data:

```go
templateData["GPUMetrics"] = monitoring.Spec.Metrics != nil
```

### OpenTelemetry Collector Configuration

The otel-collector template (`opentelemetry-collector.tmpl.yaml`) includes a conditional GPU metrics scrape job:

```yaml
{{- if .GPUMetrics }}
- job_name: 'dcgm-exporter-gpu-metrics'
...
- `job`: Set to `rhoai-gpu-metrics`
- `node`: Set from pod node name

### Recording Rules

RHOAI-specific GPU recording rules are added to provide aggregated metrics:
...
3. Query GPU metrics from Prometheus:
...
- **When Enabled**: GPU metrics appear in RHOAI Prometheus with `job="rhoai-gpu-metrics"`
- **When Disabled**: No GPU metrics collection occurs, reducing overhead

## Troubleshooting

### Common Issues

1. **No GPU metrics collected**:
