...
const (
	...
	defaultEventuallyTimeout        = 3 * time.Minute  // Set default timeout for Eventually (default is 1 second).
...
func (tc *TestContext) EnsureOperatorInstalled(nn types.NamespacedName, skipOperatorGroupCreation bool) {
	...
	}).WithTimeout(csvWaitTimeout).WithPolling(generalRetryInterval)
}
