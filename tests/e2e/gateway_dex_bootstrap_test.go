...
// connector; mockCallback satisfies that without enabling the local password DB.
...
staticClients:
  - id: %s
    ...
    redirectURIs:
      - %q
`, xksGatewayOIDCIssuerURL, xksDexPort, xksGatewayOIDCClientID, xksGatewayOIDCClientID,
...
func (tc *TestContext) waitForDexDeploymentReady(t *testing.T) {
	...
	// Poll deployment readiness via Eventually rather than EnsureDeploymentReady (point-in-time)
	// to avoid a race with pod rollout after Dex bootstrap resources are created.
