...
func (tc *TestContext) EnsureResourceDoesNotExist(opts ...ResourceOpts) {
	...
	if ro.ExpectedErrMatcher != nil {
		tc.g.Expect(err).To(ro.ExpectedErrMatcher, unexpectedErrorMismatchMsg, ro.ExpectedErrMatcher, err, ro.GVK.Kind)
	}
}
...
//
// Behavior is controlled by the following optional flags:
//   - WithExpectedErr: If provided, validates that the deletion produces the expected error.
...
func (tc *TestContext) EnsureResourceGone(opts ...ResourceOpts) {
	...
	tc.g.Eventually(func(g Gomega) {
		...
		// Validate the error if an expected error is set
		if ro.ExpectedErrMatcher != nil {
			g.Expect(err).To(ro.ExpectedErrMatcher, unexpectedErrorMismatchMsg, ro.ExpectedErrMatcher, err, ro.GVK.Kind)
			return
		}
...
func (tc *TestContext) EnsureWebhookBlocksOperation(operation func() error, operationType string, opts ...ResourceOpts) {
	...
	tc.g.Eventually(func(g Gomega) {
		...
		// Handle error validation based on WithExpectedErr or default webhook validation
		if ro.ExpectedErrMatcher != nil {
			// WithExpectedErr was specified - validate specific error type
			g.Expect(err).To(ro.ExpectedErrMatcher, "Expected specific error type but got: %v", err)
			return
		}
...
func (tc *TestContext) tryRemoveFinalizers(gvk schema.GroupVersionKind, nn types.NamespacedName) {
	...
	tc.EventuallyResourceCreatedOrUpdated(
		...
		WithIgnoreNotFound(true),
		WithExpectedErr(meta.IsNoMatchError, "IsNoMatchError"),
