...
func ensureResourceAppliedGomegaFunction(
	...
	return func(innerG Gomega) {
		...
		*u, err = applyResourceFn(ro.Obj, ro.MutateFunc).Get()
		...
		// Error handling based on failure expectation and WithExpectedErr
		if ro.ExpectedErrMatcher != nil && err != nil {
			// WithExpectedErr was specified - validate specific error type and return
			innerG.Expect(err).To(ro.ExpectedErrMatcher, "Expected specific error type but got: %v", err)
			return
		}
