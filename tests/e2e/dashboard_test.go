...
func (tc *DashboardTestCtx) ValidateHardwareProfileCreationBlockedByVAP(t *testing.T) {
	...
	// Use EventuallyResourceCreatedOrUpdated with a short timeout and expected error
	tc.EventuallyResourceCreatedOrUpdated(
		...
		WithEventuallyTimeout(2*time.Second),
		WithEventuallyPollingInterval(500*time.Millisecond),
		WithExpectedErr(errors.New("Creation and updates of HardwareProfile CRs from dashboard.opendatahub.io is not allowed")),
	)
}
...
func (tc *DashboardTestCtx) ValidateAcceleratorProfileCreationBlockedByVAP(t *testing.T) {
	...
	// Use EventuallyResourceCreatedOrUpdated with a short timeout and expected error
	tc.EventuallyResourceCreatedOrUpdated(
		WithMinimalObject(gvk.AcceleratorProfile, types.NamespacedName{Name: testAPName, Namespace: tc.AppsNamespace}),
		WithMutateFunc(
			func(obj *unstructured.Unstructured) error {
				obj.Object["spec"] = map[string]interface{}{
		...
		WithEventuallyTimeout(2*time.Second),
		WithEventuallyPollingInterval(500*time.Millisecond),
		WithExpectedErr(errors.New("Creation and updates of AcceleratorProfile CRs from dashboard.opendatahub.io is not allowed")),
	)
}
