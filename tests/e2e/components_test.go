...
func (tc *ComponentTestCtx) ValidateConfigMapDeletionRecovery(t *testing.T) {
	...
	if len(configMaps) == 0 {
		t.Logf("No ConfigMap resources found for component %s, skipping deletion recovery test", tc.GVK.Kind)
		return
	}
...
func (tc *ComponentTestCtx) ValidateServiceDeletionRecovery(t *testing.T) {
	...
	if len(services) == 0 {
		t.Logf("No Service resources found for component %s, skipping deletion recovery test", tc.GVK.Kind)
		return
	}
...
func (tc *ComponentTestCtx) ValidateRouteDeletionRecovery(t *testing.T) {
	...
	if len(routes) == 0 {
		t.Logf("No Route resources found for component %s, skipping deletion recovery test", tc.GVK.Kind)
		return
	}
...
func (tc *ComponentTestCtx) ValidateRBACDeletionRecovery(t *testing.T) {
	...
	for _, rbacType := range rbacResourceTypes {
		t.Run(rbacType.name+"DeletionRecovery", func(t *testing.T) {
			...
			if rbacType.namespace != "" {
				listOptions.Namespace = rbacType.namespace
			}
			...
			for _, resource := range existingResources {
				t.Run(rbacType.name+"_"+resource.GetName(), func(t *testing.T) {
					...
					tc.EnsureResourceDeletedThenRecreated(
						...
						WithEventuallyTimeout(tc.TestTimeouts.mediumEventuallyTimeout), // Use longer timeout for RBAC
					)
				})
			}
		})
