...
func (tc *DSCTestCtx) ValidateDSCICreation(t *testing.T) {
	...
	tc.EnsureResourceExists(
		...
		WithCondition(And(
			...
			jq.Match(`.spec.cookie.refresh == "2h0m0s"`),
			jq.Match(`.spec.ingressGateway.certificate.type == "OpenshiftDefaultIngress"`),
			jq.Match(`.spec.ingressGateway.certificate.secretName == "%s"`, gateway.DefaultGatewayTLSSecretName),
		)),
