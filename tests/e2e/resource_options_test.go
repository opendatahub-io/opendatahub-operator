...
type ResourceOptions struct {
	...
	// ExpectedErrMatcher is a Gomega matcher for error validation during resource operations.
	// Use WithExpectedErr() to set this - it creates the appropriate MatchError matcher.
...
// Examples:
//
//	WithExpectedErr("error message")                   // String matching
//	WithExpectedErr(someError)                         // Error instance matching
//	WithExpectedErr(k8serr.IsInvalid, "IsInvalid")     // Function matching
//	WithExpectedErr(ContainSubstring("not found"))     // Matcher
func WithExpectedErr(expected any, functionErrorDescription ...any) func(*ResourceOptions) {
	return func(ro *ResourceOptions) {
		ro.ExpectedErrMatcher = MatchError(expected, functionErrorDescription...)
	}
}
