...
func (tc *ModelMeshServingTestCtx) WithT(t *testing.T) *WithT {
	t.Helper()
...
func (tc *ModelMeshServingTestCtx) List(
...
func (tc *ModelMeshServingTestCtx) Get(
	gvk schema.GroupVersionKind,
	ns string,
	name string,
...
func (tc *ModelMeshServingTestCtx) Update(
	obj client.Object,
...
func (tc *ModelMeshServingTestCtx) Delete(
	gvk schema.GroupVersionKind,
	ns string,
	name string,
...
func (tc *ModelMeshServingTestCtx) MergePatch(
	obj client.Object,
	patch []byte,
) func() (*unstructured.Unstructured, error) {
	return func() (*unstructured.Unstructured, error) {
...
func (tc *ModelMeshServingTestCtx) updateComponent(fn func(dsc *dscv1.Components)) func() error {
	return func() error {
...
func modelMeshServingTestSuite(t *testing.T) {
	...
	t.Run(componentCtx.testDsc.Name, func(t *testing.T) {
		t.Run("Validate ModelMeshServing instance", componentCtx.validateModelMeshServingInstance)
		t.Run("Validate ModelMeshServing operands OwnerReferences", componentCtx.validateOperandsOwnerReferences)
		t.Run("Validate Update ModelMeshServing operands resources", componentCtx.validateUpdateModelMeshServingOperandsResources)
		// must be the latest one
		t.Run("Validate Disabling ModelMeshServing Component", componentCtx.validateModelMeshServingDisabled)
	})
}

func (tc *ModelMeshServingTestCtx) validateModelMeshServingInstance(t *testing.T) {
	g := tc.WithT(t)
...
func (tc *ModelMeshServingTestCtx) validateOperandsOwnerReferences(t *testing.T) {
	g := tc.WithT(t)
	...
	g.Eventually(
		tc.List(
			...
			client.MatchingLabels{labels.PlatformPartOf: strings.ToLower(componentApi.ModelMeshServingKind)},
		),
	).Should(And(
		Not(BeEmpty()),
		HaveEach(
			jq.Match(`.metadata.ownerReferences[0].kind == "%s"`, componentApi.ModelMeshServingKind),
		),
	))
}

func (tc *ModelMeshServingTestCtx) validateUpdateModelMeshServingOperandsResources(t *testing.T) {
	g := tc.WithT(t)
	...
	g.Expect(appDeployments.Items).ToNot(BeEmpty())

	const expectedReplica int32 = 2 // from 3 to 2
...
func (tc *ModelMeshServingTestCtx) validateModelMeshServingDisabled(t *testing.T) {
	g := tc.WithT(t)
