...
func (mr *ModelRegistryTestCtx) getInstance() (*componentsv1alpha1.ModelRegistry, error) {
	mri := componentsv1alpha1.ModelRegistry{}
	nn := types.NamespacedName{Name: componentsv1alpha1.ModelRegistryInstanceName}
...
func (mr *ModelRegistryTestCtx) validateOperandsOwnerReferences(t *testing.T) {
	...
	g.Eventually(
		mr.List(
			...
			client.MatchingLabels{labels.PlatformPartOf: strings.ToLower(componentsv1alpha1.ModelRegistryKind)},
		),
	).Should(And(
		HaveLen(1),
		HaveEach(
			jq.Match(`.metadata.ownerReferences[0].kind == "%s"`, componentsv1alpha1.ModelRegistryKind),
		),
	))
}
...
func (mr *ModelRegistryTestCtx) validateUpdateModelRegistryOperandsResources(t *testing.T) {
	...
	appDeployments, err := mr.kubeClient.AppsV1().Deployments(mr.applicationsNamespace).List(
		mr.ctx,
		metav1.ListOptions{
			LabelSelector: labels.PlatformPartOf + "=" + strings.ToLower(componentsv1alpha1.ModelRegistryKind),
		},
	)
