...
func (tc *GatewayTestCtx) ValidateGatewayInfrastructure(t *testing.T) {
	...
	tc.EnsureResourceExists(
		...
		WithCondition(And(
			jq.Match(`.metadata.ownerReferences[0].kind == "GatewayConfig"`),
	...
	tc.EnsureResourceExists(
		...
		WithCustomErrorMsg(gateway.KubeAuthProxyName+" deployment should be created and owned by "+serviceApi.GatewayConfigName+" GatewayConfig"),
	)
...
func (tc *GatewayTestCtx) ValidateDestinationRuleCreation(t *testing.T) {
	...
	tc.EnsureResourceExists(
		...
		WithCondition(And(
			jq.Match(`.spec.host == "kube-auth-proxy.openshift-ingress.svc.cluster.local"`),
	...
	if dashboardExists {
		...
		tc.EnsureResourceExists(
			...
			WithCondition(And(
				...
				jq.Match(`.spec.rules[0].backendRefs[0].port == 8443`),
				jq.Match(`.metadata.ownerReferences[0].kind == "GatewayConfig"`),
				jq.Match(`.metadata.ownerReferences[0].name == "%s"`, serviceApi.GatewayConfigName),
			)),
