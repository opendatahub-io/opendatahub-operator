...
func (tc *GatewayTestCtx) ValidateAuthConfigHashAnnotation(t *testing.T) {
	t.Helper()

	// Validate kube-auth-proxy deployment has secret-hash annotation with proper format
	tc.EnsureResourceExists(
		WithMinimalObject(gvk.Deployment, types.NamespacedName{
			Name:      gateway.KubeAuthProxyName,
			Namespace: gateway.GatewayNamespace,
		...
		WithCondition(And(
			// Annotation exists and is not null
			jq.Match(`.spec.template.metadata.annotations["%s"] != null`, SecretHashAnnotation),
			// Annotation is not empty
			jq.Match(`.spec.template.metadata.annotations["%s"] != ""`, SecretHashAnnotation),
			// Annotation has correct SHA256 hash format (64 hex characters)
			jq.Match(`.spec.template.metadata.annotations["%s"] | test("^[a-f0-9]{64}$")`, SecretHashAnnotation),
		)),
		WithCustomErrorMsg(gateway.KubeAuthProxyName+" deployment should have "+SecretHashAnnotation+" annotation with valid SHA256 hash format (64 hex characters)"),
	)

	t.Log("kube-auth-proxy deployment hash annotation validation completed successfully")
}
