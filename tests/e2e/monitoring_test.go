...
func (tc *MonitoringTestCtx) ValidatePrometheusSecureProxyAuthentication(t *testing.T) {
	...
	// Verify the prometheus-restricted deployment contains kube-rbac-proxy
	tc.EnsureResourceExists(
		WithMinimalObject(gvk.Deployment, types.NamespacedName{
			Name:      "prometheus-restricted",
			Namespace: dsci.Spec.Monitoring.Namespace,
		...
		WithCustomErrorMsg("prometheus-restricted deployment should be created and ready with kube-rbac-proxy container"),
	)
	...
	tc.EnsureResourceExists(
		...
		WithCondition(And(
			jq.Match(`.spec.to.name == "prometheus-restricted"`), // Should point to secure proxy, not direct Prometheus
			jq.Match(`.spec.tls.termination == "reencrypt"`),
			jq.Match(`.spec.tls.insecureEdgeTerminationPolicy == "Redirect"`),
		)),
