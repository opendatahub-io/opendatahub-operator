...
func (tc *KserveTestCtx) ValidateCustomCertificateCreation(t *testing.T) {
	...
	t.Log("Configuring custom self-signed certificate in DSC")
	tc.EnsureResourceCreatedOrUpdated(
		WithMinimalObject(gvk.DataScienceCluster, tc.DataScienceClusterNamespacedName),
...
func (tc *KserveTestCtx) ValidateDSCIDSCValidationInteractionForKserve(t *testing.T) {
	...
	tc.EnsureResourceExists(
		WithMinimalObject(gvk.DataScienceCluster, tc.DataScienceClusterNamespacedName),
		WithCondition(And(
			jq.Match(`.status.conditions[] | select(.type == "KserveReady" and .status == "False") | .message | test("servicemesh needs to be set to 'managed' in dsci cr"; "i")`),
			jq.Match(`.status.conditions[] | select(.type == "ComponentsReady" and .status == "False") | .message | test("some components are not ready: kserve"; "i")`),
		)),
	...
	if originalDSCI.Spec.ServiceMesh.ManagementState == operatorv1.Managed {
		t.Log("Waiting for ServiceMesh to be ready")
		tc.EnsureResourceExists(
			WithMinimalObject(gvk.DSCInitialization, tc.DSCInitializationNamespacedName),
