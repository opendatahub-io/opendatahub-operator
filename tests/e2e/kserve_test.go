...
func (c *KserveTestCtx) ValidateKserveOwnsFeatureTrackers(t *testing.T) {
	...
	for _, ft := range fts {
		g.Get(
			...
			jq.Match(`.metadata.ownerReferences | any(.kind = "%s")`, c.GVK.Kind),
...
func (c *KserveTestCtx) ValidateFeatureTrackersOwnChildren(t *testing.T) {
	...
	for _, child := range children {
		g.Get(child.gvk, child.nn).Eventually().Should(
...
func (c *KserveTestCtx) ValidateKnativeServingStructure(t *testing.T) {
	...
	g.Get(gvk.KnativeServing, types.NamespacedName{Namespace: "knative-serving", Name: "knative-serving"}).Eventually().Should(And(
