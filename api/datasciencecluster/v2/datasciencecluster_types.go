...
// +kubebuilder:validation:XValidation:rule="!has(self.trainingoperator) || !has(self.trainingoperator.managementState) || self.trainingoperator.managementState != 'Managed' || (has(oldSelf.trainingoperator) && has(oldSelf.trainingoperator.managementState) && oldSelf.trainingoperator.managementState == 'Managed')",message="TrainingOperator v1 removed in RHOAI 3.6. Set managementState to Removed to uninstall, then use Trainer v2."
