...
oc process --local -f configmap-create-model-template.yaml \
  -p REGISTERED_MODEL_NAME="mnist" \
  -p MODEL_VERSION_NAME="v1" \
  -p MODEL_ARTIFACT_NAME="mnist-onnx" \
...
oc process --local -f jobs-async-upload-uri-to-oci-template.yaml \
  ...
  -p MODEL_SYNC_DESTINATION_OCI_URI="default-route-openshift-image-registry.apps.rosa.my-cluster.019m.p3.openshiftapps.com/project3/mnist-v1:latest" \
