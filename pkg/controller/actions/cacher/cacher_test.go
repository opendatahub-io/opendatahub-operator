...
func newCacher(key cacher.CachingKeyFn) *cacher.Cacher {
	cacher := &cacher.Cacher{}
	cacher.SetKeyFn(key)
	return cacher
}
func TestCacherShouldRenderFirstRun(t *testing.T) {
	...
	res, acted, err := m.cacher.Render(m.ctx, m.render, m.rr)
...
func TestCacherShouldNotRenderSecondRun(t *testing.T) {
	...
	m.cacher.Render(m.ctx, m.render, m.rr) //nolint:errcheck
	res, acted, err := m.cacher.Render(m.ctx, m.render, m.rr)
