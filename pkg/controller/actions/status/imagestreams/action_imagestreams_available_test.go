...
func TestImageStreamsNoMatchErrorVanillaK8s(t *testing.T) {
	// On vanilla Kubernetes clusters the ImageStream CRD does not exist.
	// The action should return nil (no error) and set no condition, so the
	// component stays healthy by default.
	g := NewWithT(t)
	ctx := t.Context()
	ns := xid.New().String()
	...
	cl, err := fakeclient.New(
		fakeclient.WithObjects(newDSCI(ns)),
	...
	// No ImageStreamsAvailable condition should be set — the action returns
	// before marking any condition.
	cond := conditions.FindStatusCondition(rr.Instance.GetStatus(), status.ConditionImageStreamsAvailable)
