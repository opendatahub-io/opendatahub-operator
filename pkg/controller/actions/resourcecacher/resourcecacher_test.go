...
type testCacher struct {
	...
	doubleRes []unstructured.Unstructured
}
...
func (s *testCacher) render(ctx context.Context, rr *types.ReconciliationRequest) (any, error) {
...
func TestCacherShouldRenderFirstRun(t *testing.T) {
	...
	err := m.cacher.Render(m.ctx, m.render, m.rr)
...
func TestCacherShouldNotRenderSecondRun(t *testing.T) {
	...
	_ = m.cacher.Render(m.ctx, m.render, m.rr)
	m.resetGenerated()
	err := m.cacher.Render(m.ctx, m.render, m.rr)
...
func TestCacherShouldErrorIfRenderError(t *testing.T) {
	...
	m.On("hash", m.rr).Return(newHash(), nil).Once()
	m.On("render", m.ctx, m.rr).Return(nil, renderErr).Once()
...
func TestCacherShouldErrorIfWrongType(t *testing.T) {
	g := NewWithT(t)
	m := newTestCacher()

	m.On("hash", m.rr).Return(newHash(), nil).Once()
	m.On("render", m.ctx, m.rr).Return(map[string]string{}, nil).Once()
