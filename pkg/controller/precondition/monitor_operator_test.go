...
func TestMonitorOperator(t *testing.T) {
	...
	tests := []struct {
		...
		// when true, sets RequireCR on the OperatorConfig
		requireCR bool
		...
		{
			name: "operator CR not found with RequireCR",
			setupOperatorCR: func(_ *WithT, _ string) *unstructured.Unstructured {
				return nil
			},
			filter:              defaultTestFilter,
...
func TestMonitorOperator_MissingCRD(t *testing.T) {
	...
	t.Run("RequireCR=true still passes when CRD is missing", func(t *testing.T) {
		g := NewWithT(t)
