...
func (r *Manager) Reset() {
	...
	// Pre-populate with all registered dependents so that conditions
	// initialized by NewManager are not removed as stale when an action
	// that would re-set them does not run (e.g. preconditions fail).
	// Only conditions from non-registered sources that are truly abandoned
	// will be cleaned up.
	for _, d := range r.dependents {
		r.activeTypes[d] = struct{}{}
	}
}
...
func (r *Manager) CleanupStaleConditions() {
	...
	for _, c := range r.accessor.GetConditions() {
		...
		if _, active := r.activeTypes[c.Type]; !active {
			if RemoveStatusCondition(r.accessor, c.Type) {
				removed = true
			}
		}
	}
