...
// NodeAllocatableChangePredicate triggers on node allocatable changes
func NodeAllocatableChangePredicate() predicate.Funcs {
	return predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			return true
		},
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			// Check if schedulability changed (cordon/uncordon)
			if oldNode.Spec.Unschedulable != newNode.Spec.Unschedulable {
				return true
			}

			oldReadyCondition := findNodeConditionStatus(oldNode, corev1.NodeReady)
			newReadyCondition := findNodeConditionStatus(newNode, corev1.NodeReady)
			if oldReadyCondition != newReadyCondition {
				return true
			}

			// Check if allocatable resources changed (e.g. adding device plugins)
...
func findNodeConditionStatus(node *corev1.Node, conditionType corev1.NodeConditionType) corev1.ConditionStatus {
