...
// Domain or Subdomain fields change, ignoring other GatewayConfig changes.
// This is useful for components that need to update their params.env when the gateway domain changes.
func GatewayConfigDomainChanged() predicate.Predicate {
	return predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			domainChanged := oldGC.Spec.Domain != newGC.Spec.Domain
			subdomainChanged := oldGC.Spec.Subdomain != newGC.Spec.Subdomain

			// Only trigger if Domain or Subdomain changed
			return domainChanged || subdomainChanged
		},
		GenericFunc: func(e event.GenericEvent) bool {
			return false
		},
