...
func TestNodeAllocatableChangePredicate(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "when schedulability changes",
			oldObj: &corev1.Node{
		...
		{
			name: "when ready condition changes",
			oldObj: &corev1.Node{
		...
		{
			name: "when allocatable changes (new GPU added)",
			oldObj: &corev1.Node{
				Status: corev1.NodeStatus{
		...
		{
			name: "when nothing interesting changes",
			oldObj: &corev1.Node{
				Status: corev1.NodeStatus{
					Allocatable: corev1.ResourceList{
						corev1.ResourceCPU:    resource.MustParse("1000m"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := resources.NodeAllocatableChangePredicate().UpdateFunc(e)
	...
	t.Run("Create event returns true", func(t *testing.T) {
		g := NewWithT(t)
		e := event.CreateEvent{}
		g.Expect(resources.NodeAllocatableChangePredicate().CreateFunc(e)).To(BeTrue())
	})

	t.Run("Delete event returns true", func(t *testing.T) {
		g := NewWithT(t)
		e := event.DeleteEvent{}
		g.Expect(resources.NodeAllocatableChangePredicate().DeleteFunc(e)).To(BeTrue())
	})

	t.Run("Generic event returns false", func(t *testing.T) {
		g := NewWithT(t)
		e := event.GenericEvent{}
		g.Expect(resources.NodeAllocatableChangePredicate().GenericFunc(e)).To(BeFalse())
	})
}
