...
// forbiddenOLMv1Client simulates missing RBAC for OLMv1 resources.
type forbiddenOLMv1Client struct {
	client.Client
}

func (c *forbiddenOLMv1Client) List(ctx context.Context, list client.ObjectList, opts ...client.ListOption) error {
	if _, ok := list.(*olmv1.ClusterExtensionList); ok {
		return k8serr.NewForbidden(schema.GroupResource{
...
func (c *forbiddenOLMv1Client) Get(ctx context.Context, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
	if _, ok := obj.(*olmv1.ClusterCatalog); ok {
		return k8serr.NewForbidden(schema.GroupResource{
			Group:    olmv1.GroupVersion.Group,
...
func newOLMv1ForbiddenClient(t *testing.T) client.Client {
	t.Helper()
	g := NewWithT(t)
	scheme, err := testscheme.New()
	g.Expect(err).NotTo(HaveOccurred())
	return &forbiddenOLMv1Client{Client: fake.NewClientBuilder().WithScheme(scheme).Build()}
}
...
func TestDeleteClusterExtension(t *testing.T) {
	...
	t.Run("no-op when OLMv1 API is forbidden", func(t *testing.T) {
		cli := newOLMv1ForbiddenClient(t)
		g.Expect(cluster.DeleteClusterExtension(ctx, cli, rhoaiPackage)).To(Succeed())
	})
}
